...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xml:lang="en"
    ...
    <sch:p>The Schematron Validation Report Language is a simple language for implementations to use
        ...
        validating a document, in any order, together with other information such as which rules
        fire. </sch:p>
    ...
    <sch:pattern>
        ...
        <sch:rule abstract="true" id="childless">
            <sch:assert test="count(*) = 0"> The <sch:name/> element should not contain any
                elements. </sch:assert>
        </sch:rule>
        <sch:rule abstract="true" id="empty">
            <sch:extends rule="childless"/>
            <sch:assert test="string-length(normalize-space(.)) = 0"> The <sch:name/> element should
                be empty. </sch:assert>
        </sch:rule>
        <!-- Rules-->
        <sch:rule context="svrl:schematron-output">
            ...
            <sch:assert
                test="
                    count(svrl:text) + count(svrl:ns-prefix-in-attribute-values) +
        ...
        <sch:rule context="svrl:ns-prefix-in-attribute-values">
            ...
            <sch:assert
                test="
                    following-sibling::svrl:active-pattern
        ...
        <sch:rule context="svrl:fired-rule">
            ...
            <sch:assert
                test="
                    preceding-sibling::active-pattern |
                    preceding-sibling::svrl:fired-rule |
                    preceding-sibling::svrl:failed-assert |
                    preceding-sibling::svrl:successful-report"
                > A <sch:name/> comes after an active-pattern, an empty fired-rule, a failed-assert
                or a successful report. </sch:assert>
        ...
        <sch:rule context="svrl:failed-assert | svrl:successful-report">
            <sch:extends rule="second-level"/>
            <sch:assert test="count(svrl:diagnostic-reference) + count(svrl:text) = count(*)"> The
                <sch:name/> element should only contain a text element and diagnostic reference
                elements. </sch:assert>
            <sch:assert test="count(svrl:text) = 1"> The <sch:name/> element should only contain a
                text element. </sch:assert>
            <sch:assert
                test="
                    preceding-sibling::svrl:fired-rule |
                    preceding-sibling::svrl:failed-assert |
    ...
    <sch:pattern>
        ...
        <sch:rule context="*[@id]">
            <sch:assert test="not(preceding::*[@id=current()/@id][1])"> Id attributes should be
                unique in a document. </sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern abstract="true" id="requiredAttribute">
        <sch:title>Required Attributes</sch:title>
        <sch:rule context="$context">
            <sch:assert test="string-length($attribute) &gt; 0">The <sch:name/> element should have
                a <sch:value-of select="$attribute/name()"/> attribute.</sch:assert>
        </sch:rule>
    </sch:pattern>
